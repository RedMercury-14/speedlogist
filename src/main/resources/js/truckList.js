...
class CustomNoRowsOverlay {
	init(params) {
		...
		noRowOverlayBtn.innerText = '+ Добавить автомобиль'
		noRowOverlayBtn.addEventListener('click', () => addTruck())
...
const columnDefs = [
	...
	{ headerName: 'Марка', field: 'brandTruck', },
...
window.onload = async () => {
	...
	const addTruckBtn = document.querySelector('#addTruckBtn')
	...
	addTruckBtn.addEventListener('click', (e) => {
...
// функции управления списком машин
function addTruck() {
	const createTruckForm = document.querySelector('#createTruckForm')
	const TCfileInput = document.querySelector('#technical_certificate_file')
	const truckImgContainer = document.querySelector("#truckImageContainer")

	createTruckForm.removeEventListener('submit', updateTruckFormCallback)
	createTruckForm.addEventListener('submit', addTruckFormCallback)
	truckImgContainer.innerHTML = ''
	TCfileInput.required = true
	TCfileInput.parentElement.classList.remove('none')
	createTruckForm.reset()
	openCarModal('Создать новую машину')
}
function editTruck(data) {
	...
	createTruckForm.removeEventListener('submit', addTruckFormCallback)
...
// обработчик отправки данных формы для СОЗДАНИЯ машины
function addTruckFormCallback(e) {
	e.preventDefault()
	...
	ajaxUtils.postMultipartFformData({
		...
		successCallback: (response) => {
			if (response) {
				...
				snackbar.show('Машина создана!')
				updateTable()
				$(`#truckModal`).modal('hide')
			} else {
				snackbar.show('Произошла ошибка регистрации, исправьте данные')
			}
