...
import by.base.main.util.hcolossus.service.*;
 ...
 */
@Component
public class ColossusProcessorANDRestrictions5Sync {
...
//	private Comparator<Vehicle> vehicleComparatorFromMaxAndMass = (o1, o2) -> (o2.getWeigth() / o2.getPall() - o1.getWeigth() / o1.getPall());// сортирует от большей потребности к меньшей C УЧЁТОМ веса
    ...
    public ThreadLocal<List<Shop>> localShopsForOptimization = ThreadLocal.withInitial(ArrayList::new);
    ;
    //	private List<Shop> shopsForOptimization;

    public ThreadLocal<List<Shop>> localShopsForDelete = ThreadLocal.withInitial(ArrayList::new);
    //    private List<Shop> shopsForDelite;

    public ThreadLocal<List<Vehicle>> localTrucks = ThreadLocal.withInitial(ArrayList::new);
    ;
//    private List<Vehicle> trucks;
    ...
    public ThreadLocal<List<Vehicle>> localVehicleForDelete = ThreadLocal.withInitial(ArrayList::new);
//    private List<Vehicle> vehicleForDelete;

    public ThreadLocal<String> localStackTrace = new ThreadLocal<>();
//    private String stackTrace;

    public ThreadLocal<List<VehicleWay>> localWhiteWay = ThreadLocal.withInitial(ArrayList::new);
//    private List<VehicleWay> whiteWay;
    ...
    public ThreadLocal<List<Shop>> localShopsForAddNewNeedPall = ThreadLocal.withInitial(ArrayList::new);
...
//        vehicleForDelete = new ArrayList<Vehicle>();
        ...
        try {
            localTrucks.set(vehicleMachine.prepareVehicleListVersion2(jsonMainObject));
            List<Vehicle> syncTrucks = localTrucks.get();
        } catch (ParseException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
        }
        localTrucks.get().sort(vehicleComparatorFromMax); // сортируем от больших паллет к маньшим

        /**
         * магазины для распределения выстроены в порядке убывания потребностей
         */

        localShopsForOptimization.set(shopMachine.prepareShopList5Parameters(shopList, pallHasShops, tonnageHasShops, stock, shopsWithCrossDockingMap, pallReturn, weightDistributionList, shops));
        List<Shop> shopsForOptimization = shopMachine.prepareShopList5Parameters(shopList, pallHasShops, tonnageHasShops, stock, shopsWithCrossDockingMap, pallReturn, weightDistributionList, shops); // магазины для распределения выстроены в порядке убывания потребностей TEST

        if (localTrucks.get().get(0).getPall() >= 33.0) {// Делает так, чтобы магазины, которые входят в кроссы были сверху списка, если есть фуры
            sortedShopsHasKrossingPall(true);
        } else {
            sortedShopsHasKrossingPall(false);
        }

        localStackTrace.set("Начальное распределение магазинов слеюущее: \n");
        for (Shop s : localShopsForOptimization.get()) {
//			System.out.println("->" + s.getNumshop() + " - " + s.getDistanceFromStock() + " km - " + s.getNeedPall() + " pall");
            ...
            localStackTrace.set(localStackTrace.get() + "магазин : " + s.getNumshop() + "- входит в кросс " + answer + " - " + s.getDistanceFromStock() + " км - "
                    + s.getNeedPall() + " паллет; Забрать нужно: " + answer2 + "; " + answer3 + "\n");
        }

        System.out.println("===========");
        localStackTrace.set(localStackTrace.get() + "===========\n");

        // тут пойдёт логика самого маршрутизатора
        int i = 0; // для тестов, чтобы ограничивать цикл while ОН ЖЕ ID маршрута
        int iMax = localShopsForOptimization.get().size() * localShopsForOptimization.get().size() * 10;
        int j = 0; // итерация
        ...
        localStackTrace.set(localStackTrace.get() + "Начальное распределение машин слеюущее: \n");
        for (Vehicle v : localTrucks.get()) {
            String answer = !v.isTwiceRound() ? "НЕТ" : "ДА";
            String answer2 = !v.isClone() ? "НЕТ" : "ДА";
            localStackTrace.set(localStackTrace.get() + "Машина : " + v.getId() + "/" + v.getName() + ", паллеты : " + v.getPall() + ", второй круг : " + answer + ", клон : " + answer2 + "!\n");
        }

        //тут проверяем - в принципе хватит паллет по машинам или нет
        if (!checkPallets(localShopsForOptimization.get(), localTrucks.get())) {
            throw new FatalInsufficientPalletTruckCapacityException("Недостаточно машин для распределения всех магазинов", localShopsForOptimization.get(), localTrucks.get(), koeff);
        }
        ...
        for (Shop shop : localShopsForOptimization.get()) {
            if (shop.getPallReturn() != null) {
                for (Vehicle truck : localTrucks.get()) {
                    if (truck.getPall() >= shop.getPallReturn()) {
                        reservationTruck.put(shop.getNumshop(), truck);
                    }
                    localTrucks.get().remove(truck);
                    break;
                }
            }
        }

        while (!localTrucks.get().isEmpty()) {
            if (i == iMax) {
                localStackTrace.set(localStackTrace.get() + "Задействован лимит итераций \n");
                break;
            }
            if (localShopsForOptimization.get().isEmpty()) {
            ...
            if (!checkPallets(localShopsForOptimization.get(), localTrucks.get())) {
//				throw new InsufficientPalletTruckCapacityException("Недостаточно машин для распределения всех магазинов", localShopsForOptimization.get(), localTrucks.get(), koeff);
                localStackTrace.set(localStackTrace.get() + "Не хватает машин для распределения на маршруте с id = " + i + " \n");
            }
...
//            shopsForAddNewNeedPall = new ArrayList<Shop>();
            ...
            localShopsForOptimization.get().sort(shopComparatorDistanceMain);
            if (localTrucks.get().get(0).getPall() >= 33.0) {// Делает так, чтобы магазины, которые входят в кроссы были сверху списка, если есть фуры
                sortedShopsHasKrossingPall(true);
            } else {
                sortedShopsHasKrossingPall(false);
            ...
            Shop firstShop = localShopsForOptimization.get().remove(0);
            ...
            Double maxPallTruck = localTrucks.get().get(0).getPall();
            ...
            if (firstShop.getSpecialWeightDistribution()) {
                radiusMap = getPallHasMaxToMin(localShopsForOptimization.get(), firstShop);
            } else {
                radiusMap = getDistanceMatrixHasMin2(localShopsForOptimization.get(), firstShop);
            }
            ...
            if (firstShop.getPallReturn() == null || firstShop.getPallReturn() != null && firstShop.getPallReturn() <= localTrucks.get().get(0).getPall()) {
                virtualTruck = localTrucks.get().get(0);
                if (reservationTruck.get(firstShop.getNumshop()) != null) {
                    localTrucks.get().add(reservationTruck.remove(firstShop.getNumshop())); // возвращаем зарезервированну  машину
                }
            } else {
...
//				if (logicResult > 0 && distanceBetween <= maxDistanceInRoute) {
                if (logicResult > 0 && distanceBetween <= maxDistanceInRoute || logicResult < 0 && distanceBetween <= maxDistanceInRoute && percentFilling >= minimumPercentageOfCarFilling) {
                    localShopsForOptimization.get().remove(shop2);
                    if (shop2.getPallReturn() != null && shop2.getPallReturn() <= virtualTruck.getPall()) {
                        if (reservationTruck.get(shop2.getNumshop()) != null) {
                            localTrucks.get().add(reservationTruck.get(shop2.getNumshop()));
...
//					localTrucks.get().remove(virtualTruck);
...
//					virtualTruck.setTargetPall(totalPall);	
                ...
                if (totalPall > virtualTruck.getPall().intValue() && !isRestrictions || totalWeigth > virtualTruck.getWeigth().intValue() && !isRestrictions) {
                    localShopsForOptimization.get().add(shop2);
                    points.remove(points.size() - 1);
                }
            ...
            if (isRestrictions || isRestrictionsFirst) {
                ...
                do {
                    ...
                    localTrucks.get().stream().filter(t -> t.getPall() <= specialPall).forEach(t -> specialTrucks.add(t));
...
//						localShopsForOptimization.get().add(specialShop);
                        break;
                    }
                    List<Shop> radiusMapSpecial = new ArrayList<Shop>();
                    if (specialShop.getSpecialWeightDistribution()) {
                        radiusMapSpecial = getPallHasMaxToMin(localShopsForOptimization.get(), specialShop);
                    } else {
                        radiusMapSpecial = getDistanceMatrixHasMin2(localShopsForOptimization.get(), specialShop);
                    }
                    ...
                    if (specialShop.getPallReturn() == null || specialShop.getPallReturn() != null && specialShop.getPallReturn() <= specialTrucks.get(0).getPall()) {
                        ...
                        if (reservationTruck.get(specialShop.getNumshop()) != null) {
                            localTrucks.get().add(reservationTruck.remove(specialShop.getNumshop())); // возвращаем зарезервированную машину в общай список
                        }
...
//					virtualTruck = specialTrucks.remove(0);
                    ...
                    if (totalPall > virtualTruck.getPall().intValue() || totalWeigth > virtualTruck.getWeigth().intValue()) {
                        ...
                        localShopsForOptimization.get().addAll(extraShop);
                        points = correctRoute;
                    }
                    /**
...
//							virtualTruck.setTargetPall(totalPall);	
            ...
            if (points.size() >= 3) {//тут делаем проверку на то что не ломаный ли маршрут (типо 1700-1700)
                ...
                localTrucks.get().remove(virtualTruck); //две одинаковые строки?
                localTrucks.get().remove(virtualTruck);
...
//			optimizePoints(vehicleWayVirtual);//метод оптимизации точек маршрута
                ...
                if (vehicleWayVirtual.getVehicle().isTwiceRound() && !vehicleWayVirtual.getVehicle().isClone() && vehicleWayVirtual.calcTotalRun(matrixMachine) > 250000.0) {//если тачка помечена готовой для второго круга - создаём её клон
                    ...
                    localTrucks.get().remove(cloneTruck);
                }

                localWhiteWay.get().add(vehicleWayVirtual);
                i++;
            } else {
        ...
        localStackTrace.set(localStackTrace.get() + "-->Остановлен  на итерации " + i + ". Максимальное значение итераций в данном задании: " + iMax + ".<-- \n");

        //добавляем проблемные магазины
        localShopsForOptimization.get().addAll(problemShops);

        localStackTrace.set(localStackTrace.get() + "Результат после третьего этапа: \n");
        for (VehicleWay v : localWhiteWay.get()) {
            localStackTrace.set(localStackTrace.get() + v + "\n");
        }
        localStackTrace.set(localStackTrace.get() + "========= Свободные авто ==========\n");
        for (Vehicle v : localTrucks.get()) {
            localStackTrace.set(localStackTrace.get() + v + "\n");
        }
        localStackTrace.set(localStackTrace.get() + "+++++++++ Оставшиеся магазины +++++++++++\n");
        for (Shop v : localShopsForOptimization.get()) {
            localStackTrace.set(localStackTrace.get() + v.getNumshop() + " (" + v.getNeedPall() + ") info = " + v.getPackageShop() + "\n");
        }


        localWhiteWay.get().forEach(w -> System.out.println(w));
        System.out.println("========= Свободные авто ==========");
        localTrucks.get().forEach(t -> System.out.println(t));
        System.out.println("+++++++++ Оставшиеся магазины +++++++++++");
//		localShopsForOptimization.get().forEach(s -> System.out.println(s.toAllString()));


        Solution solution = new Solution();

        solution.setEmptyShop(localShopsForOptimization.get());
        solution.setEmptyTrucks(localTrucks.get());
        solution.setWhiteWay(localWhiteWay.get());
...
//		stackTrace = stackTrace + "Суммарный пробег маршрута: " + totalRunHasMatrix + " м.\n";
        solution.setStackTrace(localStackTrace.get());

        localShopsForOptimization.remove();
        localShopsForDelete.remove();
        localTrucks.remove();
        localVehicleForDelete.remove();
        localStackTrace.remove();
        localWhiteWay.remove();
        localShopsForAddNewNeedPall.remove();
        return solution;
    ...
    private void optimizePointsAndLastPoint(VehicleWay vehicleWayVirtual) {
        ...
        for (Shop shop : points) {
            ...
            for (Entry<Double, Shop> entry : distanceMap.entrySet()) {
...
//					vehicleWayVirtual.setTotalRun(totalRunHasMatrix);
    ...
    private void superProcessingWay(VehicleWay vehicleWayVirtual, Shop targetStock) {
        ...
        for (Shop shop : mainPoints) {
            ...
            Map<Double, Shop> radiusFromShop = getDistanceMatrixHasMinLimitParameter(localShopsForOptimization.get(), null, shop, 10000.0);
            for (Entry<Double, Shop> entry : radiusFromShop.entrySet()) {
                ...
                if (truck.getWeigth() >= allWeigth && truck.getPall() >= allPall && !newPoints.contains(nextShop)) {
                    ...
                    localShopsForOptimization.get().remove(nextShop);
                }
            }
...
//			radiusFromShop.entrySet().forEach(e -> System.out.println("++++++>> " + e.getKey() + " - " + e.getValue().toString()));			
        ...
        localShopsForOptimization.get().addAll(mainPoints);
...
//		.stream().filter(s-> s.getPallReturn()!=null).filter(s-> pallReturnInWay!= null && pallReturnInWay < s.getPallReturn()).forEach(s-> pallReturnInWay = s.getPallReturn());
        ...
        if (!vehicleWayVirtual.getVehicle().isFull()) {
            ...
            localTrucks.get().forEach(t -> System.out.println(t));
            localTrucks.get().sort(vehicleComparatorFromMin);
            for (Vehicle truck : localTrucks.get()) {
//				System.err.println(truck + " <----> " + oldTruck);
                if (truck.getWeigth() >= oldTruck.getTargetWeigth()) {
                    if (truck.getPall() >= oldTruck.getTargetPall() && truck.getPall() < pallHasOldTruck && extraPall > truck.getPall() && truck.getPall() >= pallReturnInWay) {
                        ...
                        localTrucks.get().add(oldTruck);
                        localTrucks.get().remove(truck);
                        localVehicleForDelete.get().add(truck);
                        localVehicleForDelete.get().remove(oldTruck);
                        vehicleWayVirtual.setVehicle(truck);
                        localStackTrace.set(localStackTrace.get() + "Меняем тачку с " + oldTruck.getPall() + " на "
...
//			System.out.println("-->>> changeTruckHasSmall FINISH : " + vehicleWayVirtual);
            localTrucks.get().sort(vehicleComparatorFromMax);
        }
    }
...
//			int size = processWay.size();
            ...
            Map<Double, Shop> radiusFromLastShop = getDistanceMatrixHasMinLimitParameter(localShopsForOptimization.get(), localShopsForDelete.get(), lastShopStart, 10000.0);
            for (Entry<Double, Shop> e : radiusFromLastShop.entrySet()) {
                ...
                if (e.getValue().getNeedPall() <= vehicleWayVirtual.getFreePallInVehicle()) {
                    ...
                    if (e.getValue().getNumshop() != lastShop.getNumshop() && !localShopsForDelete.get().contains(e.getValue())) {
                        ...
                        if (e.getValue().getWeight() < vehicleWayVirtual.getFreeWeigthInVehicle()) {
                            ...
                            localShopsForDelete.get().add(e.getValue());
                            localShopsForOptimization.get().remove(e.getValue());
...
//							localShopsForDelete.get().add(e.getValue());
//							localShopsForOptimization.get().remove(e.getValue());
//							localShopsForOptimization.get().add(oldShopForReplace);
//							size = processWayNew.size();
...
//							
//							localShopsForDelete.get().remove(lastShop);
//							if(!localShopsForOptimization.get().contains(lastShop)) {
//								localShopsForOptimization.get().add(lastShop);
//							}
//							
...
//	}
    ...
    private void createIdealWay(String id, Shop shop, Shop targetStock) throws Exception {
        ...
        Double maxPallTruck = localTrucks.get().get(0).getPall();
        Integer maxWeighTruck = localTrucks.get().get(0).getWeigth();
        ...
        if (shopPall == maxPallTruck && maxWeighTruck >= shopWeight) {
            ...
            Vehicle targetTruck = localTrucks.get().remove(0);
            ...
            localWhiteWay.get().add(vehicleWay);
            localShopsForDelete.get().add(shop);
            ...
            if (newShopHasPall.getNeedPall() > 0) {
                localShopsForAddNewNeedPall.get().add(newShopHasPall);
            }
...
//			System.err.println("Грузоподъемность машины: " + targetTruck.getWeigth());						
        ...
        localShopsForOptimization.get().addAll(localShopsForAddNewNeedPall.get());
    }
    ...
    private void createIdealWayAndPallRestriction(String id, Shop shop, Shop targetStock) throws Exception {
        ...
        if (pallRestrictionIdeal != null && shopPall >= pallRestrictionIdeal || pallRestrictionIdeal != null && pallRestrictionIdeal < maxPallTruck) {
            ...
            if (targetTruck == null) {
                localStackTrace.set(localStackTrace.get() + "Ограничения на магазин следующие: не более " + shop.getMaxPall() + "паллет! Машин равных или меньше данному значению не найдено!\n");
...
//				return solution;
            ...
            if (widthNewShop > targetTruck.getWeigth()) {
                ...
                localTrucks.get().remove(targetTruck);
                double newNeedPall = targetTruck.getWeigth() / (int) oneWidthPall; // определяем сколько паллет (по ср. весу) поместится в эту машину
                targetTruck.setTargetPall(newNeedPall); // загружаем этим колличеством паллет
                Double newNeedPallForShop = shopPall - newNeedPall;
                Integer finalWidthFOrTruck = (int) ((int) newNeedPall * oneWidthPall);
    ...
    public void sortedShopsHasKrossing() {
        ...
        List<Shop> sortedShops = localShopsForOptimization.get().stream()
        ...
        sortedShops.addAll(localShopsForOptimization.get().stream()
                .filter(s -> s.getKrossPolugonName() == null)
                .collect(Collectors.toList()));

        // Перезаписываем исходный список
        localShopsForOptimization.get().clear();
        localShopsForOptimization.get().addAll(sortedShops);
    }
    ...
    public void sortedShopsHasKrossingAndReturnPall() {
        ...
        List<Shop> filteredShops = localShopsForOptimization.get().stream()
        ...
        localShopsForOptimization.get().removeAll(filteredShops);

        // Добавляем оставшиеся элементы в конец отфильтрованных
        filteredShops.addAll(localShopsForOptimization.get());

        // Обновляем исходный список магазинов
        localShopsForOptimization.get().clear();
        localShopsForOptimization.get().addAll(filteredShops);
    }
...
//			System.out.println(temp);
     ...
     */
    public synchronized boolean checkPallets(List<Shop> shopsForOptimization, List<Vehicle> trucks) {
