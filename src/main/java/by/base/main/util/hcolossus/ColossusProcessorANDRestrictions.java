 ...
 */
@Component
public class ColossusProcessorANDRestrictions {
	...
	public Solution run(JSONObject jsonMainObject, List<Integer> shopList, List<Integer> pallHasShops, List<Integer> tonnageHasShops, Integer stock,
		 ...
		 */
		List<Shop> shopsForOptimization = shopMachine.prepareShopList3Parameters(shopList, pallHasShops,tonnageHasShops, stock); // магазины для распределения выстроены в порядке убывания потребностей TEST
...
//		stackTrace = stackTrace + "==================MATRIX==================\n";
		...
		int iMax = shopsForOptimization.size() * shopsForOptimization.size() * 5;
		int j = 0; // итерация
...
//						Shop newShopHasPall = new Shop(shop.getNumshop(), shop.getAddress(), shop.getLat(), shop.getLng());
				...
				if (shopPall >= maxPallTruck && pallRestrictionIdeal != null) {
					// логика создания идеального маршрута если нет ограничений
					if (shopPall <= pallRestrictionIdeal && shopPall == maxPallTruck) {
						Vehicle targetTruck = trucks.remove(0);
						targetTruck.setTargetPall(targetTruck.getPall()); // загружаем машину полностью
						List<Shop> points = new ArrayList<Shop>();
						points.add(targetStock);
...
//			}
			...
			if (shopsForDelite.isEmpty() || shopsForAddNewNeedPall.isEmpty()) {// если в листе удаления магазина есть
...
//							pointsOld.add(targetStock);
					...
					if (!vehicleWayVirtual.getVehicle().isFull()) {
						...
						Map<Double, Shop> radiusFromLastShop = getDistanceMatrixHasMinLimitParameter(
								shopsForOptimization, shopsForDelite, lastShop, 10000.0);
						for (Map.Entry<Double, Shop> e : radiusFromLastShop.entrySet()) {
							...
							if (e.getValue().getNeedPall() <= vehicleWayVirtual.getFreePallInVehicle()) {
								if (e.getValue().getNumshop() != lastShop.getNumshop()
										&& !shopsForDelite.contains(e.getValue())) {
