...
/**
 * Самый основной оптимизатор! БАЗА!
 * Изменение запрещено!
 */
@Component
public class MAINColossusProcessorVirtualWayVer2 {
	...
	private Comparator<Vehicle> vehicleComparatorFromMax = (o1, o2) -> (o2.getPall() - o1.getPall()); //сортирует от большей меньшей потребности к большей
	private Comparator<Vehicle> vehicleComparatorFromMin= (o1, o2) -> (o1.getPall() - o2.getPall()); //сортирует от большей большей потребности к меньшей
...
//			shopsForOptimization.sort(shopComparatorForIdealWay);
			shopsForOptimization.sort(shopComparatorPallOnly);
...
//				whiteWay.add(vehicleWayVirtual);
				...
				if(vehicleWayVirtual.getWay().size() > 4) {
					...
					int countShopHasOldWay = vehicleWayVirtual.getWay().size(); // колличество магазинов в базовом маршруте
					//мы говорим что нашли оптимальное количество магазинов, и единственный способ сократить расстояние - брать часть магазинов и магрута (2 магаза заменить 1)
					Double maxDIstanceHasWay = 0.0; 
					
					for (int n = 0; n < vehicleWayVirtual.getWay().size()-1; n++) {
						String key = vehicleWayVirtual.getWay().get(n).getNumshop()+"-"+vehicleWayVirtual.getWay().get(n+1).getNumshop();
...
//					vehicleWayVirtual.getWay().remove(0); // удаляем первый склад
					...
					int pallForReplase = vehicleWayVirtual.getWay().get(1).getNeedPall() + vehicleWayVirtual.getWay().get(2).getNeedPall();
					...
					if(!goodChoiceForWay.isEmpty()) {
						for (Shop shop : goodChoiceForWay) {
							...
							for (int q = 0; q < pointsTest.size()-1; q++) {
								String key = pointsTest.get(q).getNumshop()+"-"+pointsTest.get(q+1).getNumshop();
//								System.out.println(key +"   "+ vehicleWayVirtual.getId());
								distanceTest = distanceTest + matrixMachine.matrix.get(key);
							}
							
							if(distanceTest < maxDIstanceHasWay && calcPallHashHsop(pointsTest, targetStock) <= vehicleWayVirtual.getVehicle().getPall()) {
								...
								stackTrace = stackTrace+"\n его пробег составляет " + distanceTest + " против " + maxDIstanceHasWay + " \n";
							}else {
								stackTrace = stackTrace + "Магазин " + shop.getNumshop() + " не подходит т.к. ";
								stackTrace = stackTrace + distanceTest + " (пробег с новым магазином) > " + maxDIstanceHasWay + " (дистанция виртуального маршрута)\n";
							}
						}
...
//						pointsOld.add(targetStock);
				...
				if(!vehicleWayVirtual.getVehicle().isFull()) {
					...
					//берем последнюю точку, прокладываем радиус 10 км, убераем её и ищем магаз (магазы) которые могут догрузить машину
					...
					for (Map.Entry<Double, Shop> e : radiusFromLastShop.entrySet()) {
						if(e.getValue().getNeedPall() <= vehicleWayVirtual.getFreePallInVehicle()) {
							if(e.getValue().getNumshop() != lastShop.getNumshop() && !shopsForDelite.contains(e.getValue())) {
								...
								System.err.println("~~~~~~~~~~~~~~~~~СРАБОТАЛ МАКСИМАЛЬНЫЙ ДОГРУЗ~~~~~~~~~~~~~~~~~~~");
								break;
							}							
						}
					}
...
//					}
				...
				if(!vehicleWayVirtual.getVehicle().isFull()) {
					//проверяем, есть ли, гипотетически меньшая машина, для того чтобы сохранить большую
					Integer pallHasWay = calcPallHashHsop(vehicleWayVirtual.getWay(), targetStock);
					...
					for (Vehicle truck : trucks) {
						if(truck.getPall() > pallHasWay && truck.getPall() < pallHasOldTruck) {						
							...
							vehicleWayVirtual.setVehicle(truck);
							stackTrace = stackTrace + "Меняем тачку с " + oldTruck.getPall() + " на " + truck.getPall();
...
//				}
	...
	/**
	 * Mетод принимает список нераспределенных магазинов и таргетный магазин.
	 * <br>Строит мапу, от меньшей потребности к большей от таргетного магазина в рамках заданного парамерта
	 * @param shopsForOptimization лист <b>нераспределённых магазинов<b>
	 * @param targetShop
	 * @param limit  <b>ограничение по поиску<b>
	 * @return
	 */
	private Map <Double, Shop> getDistanceMatrixHasMinLimitParameter(List<Shop> shopsForOptimization, List<Shop> shopsForDelete, Shop targetShop, Double limit) {
		Map <Double, Shop> radiusMap = new TreeMap<Double, Shop>();	
		Integer numShopTarget = targetShop.getNumshop();
		for (Shop shopHasRadius : shopsForOptimization) {
			Integer numShopTargetForTest = shopHasRadius.getNumshop();
